...
public class FrontController extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		FileInputStream fis = new FileInputStream("C:\\Users\\yyj01\\OneDrive\\문서\\GitHub\\NCS\\workspace(jsp)\\14_FileUpload\\src\\com\\upload\\controller\\mapping.properties");
		...
		if(value.substring(0, 7).equals("execute")) {
			 ...
			 * 발생하고, 또 하나의 예외는 클래스나 생성자가 접근 제한자로 인해 접근할 수 없는
			 * 경우에 발생을 함. 따라서 예외 처리를 해 주어야 함.
			 * 
			 * 반환 타입은 Object 타입이므로 맞게 형변환을 해 주면 되지만, 클래스의
			 * 타입을 모르는 상태이므로 형변환을 해 줄 수가 없음. 이러한 문제를 해결하기
			 * 위해 인터페이스를 사용을 함.
